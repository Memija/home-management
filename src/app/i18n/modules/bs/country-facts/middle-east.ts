// Middle East countries water facts - BS
export const middleEastFacts = {
  // Turkey
  TR: [
    'Turska gradi više brana nego bilo koja druga zemlja osim Kine.',
    'Moreuz Bosfor povezuje Crno more s Mediteranom.',
    'Historijske cisterne u Istanbulu skladištile su vodu za milione ljudi u rimsko doba.',
    'Turski projekt GAP stvorio je 22 brane na Eufratu i Tigrisu.',
    'Turska domaćinstva troše oko 110 litara po osobi dnevno.',
    'Antički grad Hierapolis imao je napredne rimske vodovodne sisteme.',
    'Termalne vode Pamukkalea stvaraju zadivljujuće bijele terase.',
    'Turska ima preko 1.300 termalnih izvora.',
    'Jezero Van je najveće u Turskoj i veoma je alkalno.',
    'Istanbulska Bazilika cisterna mogla je primiti 80 miliona litara.',
    'Podzemni gradovi Kapadokije imali su genijalno prikupljanje vode.',
    'Turska teži da reciklira 60% otpadnih voda do 2030. godine.',
  ],

  // UAE
  AE: [
    'Dubai proizvodi 99% svoje vode putem desalinacije - najviša stopa u svijetu!',
    'UAE troše više vode po glavi stanovnika od skoro bilo koje druge zemlje.',
    'Projekat Palm Jumeirah u Dubaiju zahtijevao je 94 miliona kubnih metara pijeska i kamena.',
    'Burj Khalifa troši 946.000 litara vode dnevno za hlađenje.',
    'Stanovnici UAE troše u prosjeku 550 litara vode dnevno.',
    'Postrojenja za desalinaciju u Dubaiju proizvode 470 miliona galona vode dnevno.',
    'UAE nemaju stalne rijeke niti jezera.',
    'Džamija šeika Zajeda u Abu Dabiju koristi recikliranu vodu za svoje vrtove.',
    'Dubai uvozi 85% svoje hrane, čuvajući lokalnu vodu za osnovne potrebe.',
    'UAE su investirali 15 milijardi dolara u vodovodnu infrastrukturu od 2010. godine.',
    'Dubai Miracle Garden koristi navodnjavanje kap po kap i recikliranu vodu.',
    'UAE grade najveće svjetsko postrojenje za desalinaciju na solarni pogon.',
  ],

  // Saudi Arabia
  SA: [
    'Saudijska Arabija nema stalne rijeke niti jezera!',
    'Saudijci troše oko 265 litara vode po osobi dnevno.',
    'Kraljevina upravlja najvećim svjetskim programom desalinacije.',
    'Desalinacija obezbjeđuje 50% pitke vode u Saudijskoj Arabiji.',
    'Zemlja je izgradila preko 30 postrojenja za desalinaciju.',
    'Drevni aflaj sistemi navodnjavanja i dalje napajaju palme datula.',
    'Saudijska Arabija je prestala s uzgojem pšenice kako bi uštedjela vodu.',
    'Rub\' al Khali (Prazna četvrt) je najveća pješčana pustinja na svijetu.',
    'Voda u Džedi u potpunosti dolazi od desalinirane morske vode.',
    'Saudijska Arabija reciklira 45% prečišćenih otpadnih voda.',
    'Polje bunara Tobruk obezbjeđuje vodu za Rijad.',
    'Zemlja ulaže velika sredstva u desalinaciju na solarni pogon.',
  ],

  // Iran
  IR: [
    'Iran troši oko 250 litara vode po osobi dnevno.',
    'Zemlja ima preko 40.000 kanata (podzemnih akvadukta)!',
    'Neki iranski kanati su stari preko 3.000 godina.',
    'Jezero Urmia se smanjilo za 90% zbog prekomjerne upotrebe vode.',
    'Iran se suočava s jednom od najgorih vodenih kriza na svijetu.',
    'Karun je jedina plovna rijeka u Iranu.',
    'Perzijski vrtovi su historijski prikazivali napredno navodnjavanje.',
    'Teheran dobija vodu iz brana Karaj i Latyan.',
    'Iran ima 270 brana za skladištenje vode.',
    'Zemlja koristi 92% svoje vode za poljoprivredu.',
    'Rijeka Zayandeh u Isfahanu često potpuno presuši.',
    'Iran gradi projekte za međubasenski transfer vode.',
  ],

  // Israel
  IL: [
    'Izrael reciklira 90% svoje otpadne vode - svjetski rekord!',
    'Izraelci troše oko 137 litara vode po osobi dnevno.',
    'Izrael je bio pionir moderne tehnologije navodnjavanja kap po kap.',
    'Zemlja vrši desalinaciju 80% svoje pitke vode.',
    'Galilejsko more (jezero Kineret) je najveće slatkovodno jezero u Izraelu.',
    'Nacionalni vodonosnik Izraela prenosi vodu sa sjevera na jug.',
    'Mrtvo more je najniža tačka na Zemlji sa 430 m ispod nivoa mora.',
    'Izraelska poljoprivreda koristi samo 1,8% vode u zemlji.',
    'Voda u Tel Avivu dolazi iz postrojenja za desalinaciju.',
    'Izrael ima 5 glavnih postrojenja za desalinaciju na obali.',
    'Zemlja izvozi vodnu tehnologiju u preko 150 nacija.',
    'Izrael je pretvorio deficit vode u suficit kroz inovacije.',
  ],
};
