// English Excel integration translations
export const excel = {
  EXCEL: {
    DEFAULT_DATE: 'Date',
    DEFAULT_KITCHEN_WARM: 'Kitchen Warm Water',
    DEFAULT_KITCHEN_COLD: 'Kitchen Cold Water',
    DEFAULT_BATHROOM_WARM: 'Bathroom Warm Water',
    DEFAULT_BATHROOM_COLD: 'Bathroom Cold Water',
    DEFAULT_LIVING_ROOM: 'Living Room',
    DEFAULT_BEDROOM: 'Bedroom',
    DEFAULT_KITCHEN: 'Kitchen',
    DEFAULT_BATHROOM: 'Bathroom',
    VALIDATION_REQUIRED: 'Column name is required',
    VALIDATION_REQUIRED_HINT: 'Column names must be text strings, not numbers or boolean values',
    VALIDATION_MUST_BE_STRING: 'must be a text string',
    VALIDATION_GOT_BOOLEAN: 'received a boolean value (true/false) instead of text',
    VALIDATION_GOT_NUMBER: 'received a number instead of text',
    VALIDATION_GOT_NULL: 'value is missing or null',
    VALIDATION_TOO_LONG: 'Column name must be 255 characters or less',
    VALIDATION_TOO_LONG_HINT: 'Column names must be 255 characters or less',
    VALIDATION_NO_WHITESPACE: 'Column name cannot have leading or trailing spaces',
    VALIDATION_NO_WHITESPACE_HINT: 'Remove leading and trailing spaces from column names',
    VALIDATION_INVALID_CHARS: 'Column name cannot contain: [ ] * / \\ ? :',
    VALIDATION_INVALID_CHARS_HINT: 'Remove the characters [ ] * / \\ ? : from column names',
    VALIDATION_DUPLICATES: 'Column names must be unique within each section',
    VALIDATION_DUPLICATES_HINT: 'Each column name must be different - check for duplicate values',
    VALIDATION_FORM_INVALID: 'Please fix validation errors before saving',
  },
};
