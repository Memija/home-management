...
export class ConsumptionDataService {
    ...
    // Filter State
    readonly filterState = signal<{ year: number | null; month: number | null; date: string | null }>({
    ...
    readonly filteredRecords = computed(() => {
        ...
        return this.records().filter(record => {
            ...
            if (state.year !== null && recordDate.getFullYear() !== state.year) return false;
            if (state.month !== null && recordDate.getMonth() !== state.month) return false;
            if (state.date !== null && recordDate.toISOString().split('T')[0] !== state.date) return false;
            return true;
        });
    });
    ...
    readonly isFilterActive = computed(() => {
        const state = this.filterState();
        return state.year !== null || state.month !== null || state.date !== null;
    });
    ...
    // --- Filter Helpers ---
    updateFilterState(state: { year: number | null; month: number | null; date: string | null }) {
        this.filterState.set(state);
    }

    getFilterSuffix(): string {
        const state = this.filterState();
        if (state.date) return `_${state.date}`;
        if (state.year && state.month !== null) return `_${state.year}_${String(state.month + 1).padStart(2, '0')}`;
        if (state.year) return `_${state.year}`;
        return '';
    }

    countRecordsOutsideFilter(records: ConsumptionRecord[]): number {
        const state = this.filterState();
        let outsideCount = 0;
        for (const record of records) {
            const recordDate = new Date(record.date);
            if (state.year !== null && recordDate.getFullYear() !== state.year) { outsideCount++; continue; }
            if (state.month !== null && recordDate.getMonth() !== state.month) { outsideCount++; continue; }
            if (state.date !== null && recordDate.toISOString().split('T')[0] !== state.date) { outsideCount++; continue; }
        }
        return outsideCount;
    }
    ...
    async exportData() {
        try {
            ...
            const filename = `water_consumption${suffix}.json`;
    ...
    async exportToExcel() {
        try {
            this.isExporting.set(true);
            const suffix = this.getFilterSuffix();
            const filename = `water_consumption${suffix}.xlsx`;
