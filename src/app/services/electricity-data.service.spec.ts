...
describe('ElectricityDataService', () => {
  ...
  describe('CRUD Operations', () => {
    ...
    it('should update an existing record with the same date', async () => {
      ...
      expect(
        service.records().find((r) => new Date(r.date).toISOString().split('T')[0] === '2023-01-01')
          ?.value,
      ).toBe(999);
    });
  ...
  describe('Error Handling', () => {
    ...
    it('should get JSON error instructions', async () => {
      ...
      fileStorageServiceMock.importFromFile.mockRejectedValue(new Error('Parse error'));
      importValidationServiceMock.getJsonErrorInstructions.mockReturnValue([
        'Instruction 1',
        'Instruction 2',
      ]);
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle records with same date but different times', async () => {
      ...
      const recordsOnDate = service
        .records()
        .filter((r) => new Date(r.date).toISOString().split('T')[0] === '2023-01-01');
