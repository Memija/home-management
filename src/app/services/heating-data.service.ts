...
export class HeatingDataService {
   ...
   */
  async saveRecord(
    newRecord: DynamicHeatingRecord,
    editingRecord: DynamicHeatingRecord | null,
  ): Promise<void> {
    if (editingRecord) {
      this.records.update((records) =>
        records.map((r) => (r.date.getTime() === editingRecord.date.getTime() ? newRecord : r)),
      );
    } else {
  ...
  async deleteRecord(record: DynamicHeatingRecord): Promise<void> {
    this.records.update((records) =>
      records.filter((r) => r.date.getTime() !== record.date.getTime()),
    );
    await this.storage.save('heating_consumption_records', this.records());
    this.notificationService.setHeatingRecords(this.records());
  }
  ...
  async confirmImport(): Promise<void> {
    ...
    try {
      ...
      const result = this.importValidationService.validateHeatingJsonImport(
        data as unknown[],
        expectedRoomIds,
      );
      ...
      if (skippedCount > 0) {
        const key =
          skippedCount === 1
            ? 'HEATING.IMPORT_PLACEHOLDER_SKIPPED_SINGULAR'
            : 'HEATING.IMPORT_PLACEHOLDER_SKIPPED_PLURAL';
        this.showWarning(
  ...
  async importFromExcel(event: Event): Promise<void> {
    ...
    try {
      ...
      if (missingColumns.length > 0) {
        warnings.push(
          this.languageService.translate('HOME.MISSING_COLUMNS') + ': ' + missingColumns.join(', '),
        );
      }
      if (skippedCount > 0) {
        const key =
