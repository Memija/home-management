...
describe('MeterReaderService', () => {
  ...
  describe('extractNumbers (private)', () => {
    it('should extract normal numbers', () => {
      const numbers = service['extractNumbers']('12345');
      expect(numbers).toEqual([12345]);
    });

    it('should extract European format numbers', () => {
      const numbers = service['extractNumbers']('1.234,56');
    ...
    it('should handle multiple thousand separators', () => {
      const numbers1 = service['extractNumbers']('1.234.567');
      expect(numbers1).toEqual([1234567]);

      const numbers2 = service['extractNumbers']('1,234,567');
      expect(numbers2).toEqual([1234567]);
    });

    it('should handle OCR misreads', () => {
      // O -> 0, l -> 1, Z -> 2, S -> 5, B -> 8
      const numbers = service['extractNumbers']('OlZSB');
      expect(numbers).toEqual([1258]); // 01258 parsed as float is 1258
    });

    it('should handle multiple numbers in text', () => {
      const numbers = service['extractNumbers']('12345.6 789');
      expect(numbers).toEqual([12345.6, 789]);
    });

    it('should ignore punctuation only text', () => {
      const numbers = service['extractNumbers']('. , .,');
      expect(numbers).toEqual([]);
    });

    it('should handle empty or null string', () => {
      expect(service['extractNumbers']('')).toEqual([]);
      expect(service['extractNumbers'](null as unknown as string)).toEqual([]);
    });
  });
  ...
  describe('scoreAndSelectBest (private)', () => {
    it('should select best by confidence when isCropped is true', () => {
      ...
      const result = (
        service as unknown as {
          scoreAndSelectBest: (c: typeof candidates, isCropped: boolean) => number;
        }
      )['scoreAndSelectBest'](candidates, true);
    ...
    it('should apply centrality and length heuristics when isCropped is false', () => {
      ...
      const result = service['scoreAndSelectBest'](candidates, false);
    ...
    it('should return null if no candidates', () => {
      expect(service['scoreAndSelectBest']([], false)).toBeNull();
      expect(service['scoreAndSelectBest']([], true)).toBeNull();
    });
  });
  ...
  describe('readMeter', () => {
      ...
      mockWorker = {
        ...
        recognize: vi.fn().mockResolvedValue({
          data: {
            text: '12345',
            confidence: 85,
            words: [{ text: '12345', confidence: 85, bbox: { x0: 10, y0: 10, x1: 50, y1: 20 } }],
          },
        }),
        terminate: vi.fn().mockResolvedValue(undefined),
      };

      const serviceAny = service as unknown as Record<string, (...args: unknown[]) => unknown>;
      vi.spyOn(serviceAny, 'getWorker').mockResolvedValue(mockWorker);
      vi.spyOn(serviceAny, 'prepareImageData').mockResolvedValue({
        imageData: 'data:image/png;base64,mock',
        imgWidth: 100,
        imgHeight: 100,
      });
      vi.spyOn(serviceAny, 'preprocessContrast').mockResolvedValue(
        'data:image/png;base64,mock-contrast',
      );
      vi.spyOn(serviceAny, 'preprocessInverted').mockResolvedValue(
        'data:image/png;base64,mock-inverted',
      );
    });
    ...
    it('should process cropped image properly with 2 passes', async () => {
      ...
      expect(service['prepareImageData']).toHaveBeenCalledWith('data:image/png;base64,mock', true);
    ...
    it('should process full image properly with 3 passes', async () => {
      ...
      expect(service['prepareImageData']).toHaveBeenCalledWith(mockFile, false);
  ...
  describe('dispose', () => {
    it('should terminate the tesseract worker if it exists', async () => {
      ...
      service['tesseractWorker'] = mockWorker;

      await service.dispose();

      expect(mockWorker.terminate).toHaveBeenCalled();
      expect(service['tesseractWorker']).toBeNull();
    });
  ...
  describe('runOcrPass (private)', () => {
    it('should configure worker and return OcrPassResult', async () => {
      ...
      const result = await service['runOcrPass']('imgData', '0123', '7');
    ...
    it('should handle undefined words in recognize output', async () => {
      ...
      const serviceAny = service as unknown as Record<string, (...args: unknown[]) => unknown>;
      vi.spyOn(serviceAny, 'getWorker').mockResolvedValue(mockWorker);
      const result = await service['runOcrPass']('imgData');
  ...
  describe('prepareImageData (private)', () => {
    it('should load image dimensions', async () => {
      const serviceAny = service as unknown as Record<string, (...args: unknown[]) => unknown>;
      vi.spyOn(serviceAny, 'fileToDataUrl').mockResolvedValue('data:image/png;base64,mock');
      vi.spyOn(serviceAny, 'upscaleImage').mockResolvedValue('data:image/png;base64,mock');
      ...
      const result = await service['prepareImageData']('data', false);
  ...
  describe('preprocessing and file utilities (private)', () => {
    it('should run preprocessContrast and return data url', async () => {
      ...
      const result = await service['preprocessContrast']('data');
    ...
    it('should run preprocessInverted and return data url', async () => {
      ...
      const result = await service['preprocessInverted']('data');
    ...
    it('should run upscaleImage and return data url', async () => {
      ...
      const result = await service['upscaleImage']('data', 100);
  ...
  describe('getWorker (private)', () => {
    it('should return existing worker if already initialized', async () => {
      ...
      service['tesseractWorker'] = mockWorker;
      const worker = await service['getWorker']();
